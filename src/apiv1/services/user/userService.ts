...
class UserService {
    ...
    public async activateUser(active_token: string) {
    ...
    public async getListUser(filter, select) {
        const users = await UserModel.find(filter, select);
        return users;
    }

    public async getUser(id, select) {
    ...
    public async editProfile(_id, update) {
        return await UserModel.findByIdAndUpdate(_id, update, {
            new: true,
        });
    }

    public async changePassword(_id, password, oldPassword, pass) {
    ...
    public async sendResetPassword(param: string) {
        const user = await UserModel.findOne(
        ...
        if (!user)
            throw new Error(
                "Không tìm thấy tài khoản nào có username hoặc email là " +
                    param
            );
        const now = Date.now();
        const token = jwt.sign(
    ...
    public async resetPassword(password, reset_password_token) {
    ...
    public async updateGroupUser(id, groups) {
        const user = await UserModel.findById(
            id,
            "_id fullname username groups"
        );
        if (!user)
